...
if (isset($_POST['user']) && isset($_POST['pass'])) {
  if (!isset($_SESSION['pass_error']) || !$_SESSION['pass_error']) $_SESSION['pass_error'] = 0;
  ...
  if ($verifycode == 1 && (!$code || strtolower($code) != $_SESSION['vc_code'])) {
    ...
    $city = '';
    $md5pass = md5($pass);
    $agentrow = $DB->getRow("SELECT * FROM pre_agent WHERE agent_name='{$user}' and agent_pass='{$md5pass}' and agent_status=1 limit 1");
    if (empty($agentrow)) {
      exit("<script language='javascript'>alert('用户名或密码不正确！');history.go(-1);</script>");
    } else {
