...
class Channel
{
	...
	static public function getSubmitInfo($typeid, $typename, $gid, $money)
	{
		...
		$info = $DB->getColumn("SELECT info FROM pre_group WHERE gid = '$gid' LIMIT 1");
		if (empty($info)) {
			return false;
		}

		$infoArr = json_decode($info, true);
		$groupinfo = $infoArr[$typeid];
