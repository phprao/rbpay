	...
	<div class="col-xs-12 col-sm-10 col-lg-8 center-block" style="float: none;">
		...
		if ($my == 'add') {
			...
			foreach ($agent as $k => $v) {
				$agents .= '<option value="' . $v['id'] . '"> ' . $v['name'] . '</option>';
			}
			...
			foreach ($agent as $k => $v) {
				if ($row['agent_id'] == $v['id']) {
					$selected = " selected";
				} else {
					$selected = " ";
				}
				$agents .= '<option value="' . $v['id'] . '"' . $selected . '>' . $v['name'] . '</option>';
			}
			...
			$agent_pay_rate = sprintf('%.2f', $_POST['agent_pay_rate']);
			$agent_withdraw_rate = sprintf('%.2f', $_POST['agent_withdraw_rate']);

			if ($phone == NULL) {
				showmsg('手机号不能为空!', 3);
			} else {
				$key = random(32);
				$sql = "INSERT INTO `pre_user` (`key`, `addtime`, `phone`, `pay`, `withdraw`, `pay_rate`, `pay_rate_bank`, `status`, `ip_white_list`, `withdraw_rate`, `withdraw_rate_bank`, `pay_channel`, `remark`, `pay_min`, `pay_max`, `withdraw_min`, `withdraw_max`, `agent_id`, `agent_pay_rate`, `agent_withdraw_rate`) VALUES (:key, NOW(), :phone, :pay, :withdraw, :pay_rate, :pay_rate_bank, :status, :ip_white_list, :withdraw_rate, :withdraw_rate_bank, :pay_channel, :remark, :pay_min, :pay_max, :withdraw_min, :withdraw_max, :agent_id, :agent_pay_rate, :agent_withdraw_rate)";
				$data = [':key' => $key, ':phone' => $phone, ':pay' => $pay, ':withdraw' => $withdraw, ':pay_rate' => $pay_rate, ':pay_rate_bank' => $pay_rate_bank, ':status' => $status, ':ip_white_list' => $ip_white_list, ':withdraw_rate' => $withdraw_rate, ':withdraw_rate_bank' => $withdraw_rate_bank, ':pay_channel' => $pay_channel, ':remark' => $remark, ':pay_min' => $pay_min, ':pay_max' => $pay_max, ':withdraw_min' => $withdraw_min, ':withdraw_max' => $withdraw_max, ':agent_id' => $agent_id, ':agent_pay_rate' => $agent_pay_rate, ':agent_withdraw_rate' => $agent_withdraw_rate];
			...
			if ($phone == NULL) {
				...
				$sql = "update `pre_user` set `phone` = :phone, `pay` = :pay, `withdraw` = :withdraw, `status` = :status, `ip_white_list` = :ip_white_list, `withdraw_rate` = :withdraw_rate, `withdraw_rate_bank` = :withdraw_rate_bank, `pay_rate` = :pay_rate, `pay_rate_bank` = :pay_rate_bank, `pay_channel` = :pay_channel,`remark` = :remark, `pay_min` = :pay_min, `pay_max` = :pay_max, `withdraw_min` = :withdraw_min, `withdraw_max` = :withdraw_max, `agent_id` = :agent_id, `agent_pay_rate` = :agent_pay_rate, `agent_withdraw_rate` = :agent_withdraw_rate where `uid` = :uid";

				$data = [':phone' => $phone, ':pay' => $pay, ':withdraw' => $withdraw, ':status' => $status, ':ip_white_list' => $ip_white_list, ':withdraw_rate' => $withdraw_rate, ':withdraw_rate_bank' => $withdraw_rate_bank, ':pay_rate' => $pay_rate, ':pay_rate_bank' => $pay_rate_bank, ':pay_channel' => $pay_channel, ':remark' => $remark, ':pay_min' => $pay_min, ':pay_max' => $pay_max, ':withdraw_min' => $withdraw_min, ':withdraw_max' => $withdraw_max, ':agent_id' => $agent_id, ':agent_pay_rate' => $agent_pay_rate, ':agent_withdraw_rate' => $agent_withdraw_rate, ':uid' => $uid];
